...
<xsl:template name="verb-stem-entry">
  ...
  <xsl:if test="string-length($lemma)&gt;0 and
                ...
                string-length($class)&gt;0">
    <xsl:variable name="verb-stem">
...
<!-- verb compounding stems -->
<xsl:template name="verb-comp-stem-entry">
  <xsl:param name="lemma"
             select="@lemma"/>
  <xsl:param name="stem"
             select="@stem"/>
  ...
  <xsl:if test="string-length($lemma)&gt;0">
    <xsl:choose>
      <xsl:when test="$frequency&gt;=$minimal-frequency">
        ...
        <xsl:choose>
          <xsl:when test="string-length($stem)&gt;0">
            <xsl:variable name="segmented-stem"
                          select="n:segment($verb-stem,$stem)"/>
            <xsl:value-of select="n:pair($segmented-lemma,$segmented-stem)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="$segmented-lemma"/>
          </xsl:otherwise>
        </xsl:choose>
        <xsl:text>&lt;V&gt;</xsl:text>
        <xsl:text>&lt;comp&gt;</xsl:text>
...
<!-- verb derivation stems -->
<xsl:template name="verb-der-stem-entry">
  <xsl:param name="lemma"
             select="@lemma"/>
  <xsl:param name="stem"
             select="@stem"/>
